...
/// Read `PrimaryInterface` and `Router` from a `State:/Network/Global/IPv*` dict.
fn global_state(store: &SCDynamicStore, key: &str) -> Option<(String, String)> {
    let dict = store
    ...
    Some((get("PrimaryInterface")?, get("Router")?))
}

pub fn primary_v4() -> anyhow::Result<PrimaryV4> {
    let store = store();
    let (ifname, router) = global_state(&store, "State:/Network/Global/IPv4")
    ...
    Ok(PrimaryV4 { ifname, router })
}

pub fn primary_v6() -> anyhow::Result<PrimaryV6> {
    let store = store();
    let (ifname, router) = global_state(&store, "State:/Network/Global/IPv6")
