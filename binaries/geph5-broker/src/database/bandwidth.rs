...
pub async fn bw_consumption(user_id: i32) -> anyhow::Result<Option<BwConsumptionInfo>> {
    let pair: Option<(i32, i32, f64)> = sqlx::query_as("select (renew_mb - (mb_limit - mb_used)), renew_mb, extract(epoch from renew_date)::bigint from bw_usage natural join bw_limits where id = $1").bind(user_id).fetch_optional(&*POSTGRES).await?;
